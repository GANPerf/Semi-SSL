...
def step4(classifier, criterions, dataset_loaders, device, model, optimizer,
          scheduler):
...
def train(args, model, model_ce, model_moco, classifier, classifier_ce, dataset_loaders, optimizer_ce, optimizer_moco,
          scheduler_ce, scheduler_moco, device=None, writer=None, model_path=None):
    len_labeled = len(dataset_loaders["train"])
    iter_labeled = iter(dataset_loaders["train"])
    is_loop = True
...
def main():
    ...
    # Train model
    train(args, model, model_ce, model_moco, classifier, classifier_ce, dataset_loaders, optimizer_ce, optimizer_moco,
