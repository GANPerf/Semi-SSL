...
def step3(args, classifier_ce, dataset_loaders, device, model_moco, model_ce):  # first mode for cycle, second for psuedo
    print('step3 starts')
    ...
    with torch.no_grad():
        ...
        df_unlabeled_cluster = pd.DataFrame(
            {'image': arr_path, 'real_label': label, 'cluster_label': cluster, 'pseudo_label': pseudo_label,
             'confidence_unlabeled': confidence})
        df_unlabeled_cluster.to_csv('unlabeled_cluster' + str(args.class_num) + str(args.label_ratio)+ str(args.confidence) + '.csv', index=False)
...
def step2(criterions, dataset_loaders, device, iter_labeled, len_labeled, model_moco, optimizer_moco,
    ...
    for iter_num in range(1, 10000 + 1):  # args.max_iter + 1   10000 is enough for convergence.
        model_moco.train(True)
        classifier_step2.train(True)
        ...
        ## For Labeled Data
        PGC_logit_labeled, PGC_label_labeled, feat_labeled = model_moco(img_labeled_q, img_labeled_k, label)

        PGC_loss_labeled = criterions['KLDiv'](PGC_logit_labeled,
                                               PGC_label_labeled)  # Contrastive loss for instances with the same labels

        out = classifier_step2(feat_labeled)
...
def step4(classifier_ce, criterions, dataset_loaders, device, model_ce, optimizer_ce, scheduler_ce):
...
def select_unlabel_data(args):
    ...
    for i in range(arr.shape[0]):
        if float(arr[i, 4]) >= args.confidence:
            select_total_unlabel_data = np.concatenate((select_total_unlabel_data, arr[i, :].reshape(1, -1)), axis=0)
