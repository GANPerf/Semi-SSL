...
def step2(criterions, dataset_loaders, device, iter_labeled, len_labeled, model, optimizer,
    ...
    for iter_num in range(1, 200 + 1):  # args.max_iter + 1   10000 is enough for convergence.
        model.train(True)
...
def select_unlabel_data(args):
    ...
    while j <= args.num_of_cluster and j <= max(list(map(int, number))):
        ...
        for i in range(sub_arr.shape[0]):
            if float(sub_arr[i, 4]) >= args.confidence:  #if sub_arr[i, 3] == most_psuedo_label and float(sub_arr[i, 4]) >= args.confidence:
